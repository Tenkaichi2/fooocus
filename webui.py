        ...
        if len(worker_outputs.value) > 0:
            ...
            if flag == 'preview':
                ...
                yield gr.update(visible=True, value=modules.html.make_progress_html(percentage, title)), \
                    gr.update(visible=True, value=image) if image is not None and not (modules.path.default_black_out_nsfw and modules.path.default_hide_preview_if_black_out_nsfw) else gr.update(), \
                    gr.update(), \
                    gr.update(visible=False)
            if flag == 'results':
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1, visible=modules.path.default_advanced_checkbox) as advanced_column:
            with gr.Tab(label='Setting'):
                ...
                image_number = gr.Slider(label='Image Number', minimum=1, maximum=modules.path.default_max_image_number, step=1, value=modules.path.default_image_number)
                ...
                if not modules.path.default_disable_log:
